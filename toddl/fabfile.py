        ...
        "letsencrypt": [
            {
                ...
                "filename": "letsencrypt.toddl.conf",
        ...
        "nginx": [
            {
                ...
                "filename": "app.toddl.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "pattern": "app.toddl.proxy_django.include",
                "filename": "app.toddl.proxy_django.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "pattern": "%(target)s.ssl",
                "filename": "ssl.toddl.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "default_site": True,
...
def deploy(id=None, silent=False, force=False, auto_nginx=True):
    ...
    app_patterns = [r' toddl/app', r' toddl/static',
                    r' toddl/settings', r' toddl/package.json']
    ...
    if crontab_changed:
        with cd(env.code_dir):
            sudo('cp deploy/crontab.conf /etc/cron.d/toddl')
...
def setup_server(id=None):
    ...
    # Create database
    sudo('echo "CREATE DATABASE toddl; '
         '      CREATE USER toddl WITH password \'{db_password}\'; '
         '      GRANT ALL PRIVILEGES ON DATABASE toddl to toddl;" '
         '| docker exec -i postgres-10 psql -U postgres'.format(db_password=db_password))

    # Upload local settings
    put(local_path=StringIO(local_settings), remote_path=env.code_dir + '/toddl/settings/local.py', use_sudo=True)

    # Create log dir
    sudo('mkdir -p /var/log/toddl/')
    ...
    sudo('touch %s/toddl/app/webpack-stats.json' % env.code_dir)
    ...
    # Copy logrotate conf
    with cd(env.code_dir):
        sudo('cp deploy/logrotate.conf /etc/logrotate.d/toddl')
...
def collectstatic(npm_build=True):
    if npm_build:
        docker_compose_run('node', 'npm run build', name='toddl_npm_build')
...
def docker_compose_run(service, cmd='', name='toddl_tmp'):
...
def ensure_docker_networks():
    ...
    ensure_docker_network_exists('toddl_default', [], internal=False)
    ensure_docker_network_exists('toddl_nginx', ['nginx'])
    ensure_docker_network_exists('toddl_postgres', ['postgres-10'])
